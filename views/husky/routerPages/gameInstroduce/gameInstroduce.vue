<template>
	<div class="gameInstroduce" v-wechat-title="this.title">
		...
		<div class="fixed" v-if="showBtn">
			<div class="btn">
				<control-btn :btnActive="true" btnText="已读,开始测评" class="submitBtn" @click.native.prevent="goToTest()"></control-btn>
			</div>
		</div>
...
<script>
	...
	import {
		gameSubject,getMatchTask,getExecute
	} from '../../../../service/api.js';
	...
	export default {
		...
		methods: {
			async init() {
				let data = await gameSubject(this.matchId);
