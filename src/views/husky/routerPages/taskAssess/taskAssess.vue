    ...
    <keep-alive>
        <div class="task" v-wechat-title="this.title">
            ...
            <template v-if="isTopic">
                ...
                v-on:toLastPart="toLastPart()" v-on:toNextPart="toNextPart()" v-on:questionNO="questionNO" v-on:submit="submitTopic()" v-on:itemIndex="sureIndex"
                v-on:stopCountdown="stopCountdown()" v-on:countDownAgain="countDownAgain()"></topic>
            </template>
            <template v-if="isQuit">
                <submit-page v-on:continueTask="continueTask()" v-on:quit="quit()"></submit-page>
            </template>
        </div>
    </keep-alive>
</template>
<script>
    ...
    import SubmitPage from "@/components/content/submitPage.vue";
    ...
    export default {
        ...
        methods: {
            ...
            continueTask(){
                ...
                if(this.turnBack == 1){
                    this.isTask = true;
                }else if(this.turnBack == 2){
                    this.isPart = true;
                }else if(this.turnBack == 3){
                    this.isTopic = true;
                }
            },
            quit(){
